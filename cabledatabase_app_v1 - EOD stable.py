...
def create_main_layout():
    ...
    # Create the menu bar with specific parameters
    menu_bar = sg.Menu(
        menu_def, 
        key='-MENU-',
        tearoff=False,
    ...
    # Left panel - Filters with enable_events=True for all inputs
    ...
    layout = [
        [menu_bar],  # Menu must be the first row
...
def apply_filters(df, values):
    ...
    print("Starting filter application...")  # Debug print
    filtered_df = df.copy()
    ...
    if values['-NUM-START-']:
        ...
        try:
            num_start = int(values['-NUM-START-'])  # Convert to integer
    ...
    if values['-NUM-END-']:
        ...
        try:
            num_end = int(values['-NUM-END-'])  # Convert to integer
...
def main():
    ...
    try:
        ...
        window = sg.Window(
            ...
            keep_on_top=False,  # Make sure this is False
        ...
        while True:
            ...
            try:
                # Add debug print for events
                if event != '__TIMEOUT__':  # Don't print timeout events
                ...
                elif event == 'Filter' or event.endswith('_Enter'):
                    print("Applying filters...")  # Debug print
                    filtered_df = apply_filters(df, values)
                    print(f"Filtered data has {len(filtered_df)} rows")  # Debug print
                    window['-TABLE-'].update(values=filtered_df.values.tolist())

                elif event == 'Clear Filter':
                    print("Clearing filters...")  # Debug print
