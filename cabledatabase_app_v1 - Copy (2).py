...
def create_main_layout():
    ...
    right_click_menu = [[], ['Hide Column', 'Show All Columns', '---', 'Sort Ascending', 'Sort Descending']]
        ...
        while True:
            ...
            try:
                ...
                if event == 'Hide Column':
                    if values['-TABLE-']:  # If a row is selected
                        col_num = values['-TABLE-'][0]  # Get the selected column number
                        visible_cols = list(window['-TABLE-'].VisibleColumns)
                        visible_cols[col_num] = False
