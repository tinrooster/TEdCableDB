...
def create_main_layout():
    ...
    # Correct column headings and right-click menu
    headers = ['NUMBER', 'DWG', 'ORIGIN', 'DEST', 'Alternate Dwg', 'Wire Type', 'Length', 'Note', 'Project ID']
    
    # Separate display and data operations in right-click menu
    right_click_menu = [[], [
        'Display Options', ['Hide Column', 'Show All Columns'],
        'Sort Options', ['Sort Ascending', 'Sort Descending']
    ]]
...
def main():
    ...
    try:
        ...
        while True:
            ...
            try:
                # Handle column header clicks for sorting (keeps the data and display in sync)
                if isinstance(event, tuple):
                    if event[0] == '-TABLE-' and event[2][1] is not None:
                        col_num = event[2][1]
                        ...
                        # Sort the dataframe and update display
                        df = df.sort_values(by=col_name, ascending=main.sort_ascending)
                        window['-TABLE-'].update(values=df.values.tolist())
                        continue

                # Handle display-only operations (column visibility)
                if event == 'Hide Column':
                    if values['-TABLE-']:
                        selected_row = values['-TABLE-'][0]
                        selected_col = window['-TABLE-'].SelectedColumns[0] if window['-TABLE-'].SelectedColumns else None
                        visible_cols = list(window['-TABLE-'].VisibleColumns)
                        visible_cols[selected_col] = False
                        window['-TABLE-'].update(visible_column_map=visible_cols)
                
                elif event == 'Show All Columns':
                    window['-TABLE-'].update(visible_column_map=[True] * len(headers))
                
                # Handle data operations (sorting)
                elif event == 'Sort Ascending':
                    if window['-TABLE-'].SelectedColumns:
                        col_num = window['-TABLE-'].SelectedColumns[0]
                        col_name = df.columns[col_num]
